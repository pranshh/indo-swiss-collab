...
printo('Downloaded ',nrow(publications_df),' works from openAlex in this period.')
...
publications_df = mutate(publications_df, rownum = row_number(), id = clean_id(id), doi = clean_id(doi), source_id = clean_id(source_id), host_organization = clean_id(host_organization))

auth100 = publications_df %>% filter(map_int(authorships, nrow) == 100) %>% pull(rownum)

if(length(auth100) > 0) {
  printo(length(auth100), ' of these publications have more than 100 authors. Getting their details separately from openAlex.')
  for(i in auth100) {
...
```{r Organise and save openalex data}
...
# 2. FAST JSON STRING PROCESSING FUNCTIONS
# These work directly on JSON strings - much faster than parsing and re-processing
...
cat("save_cleaned_data(cleaned_data)\n")

cat("\n=== TO VALIDATE CLEANED DATA, RUN ===\n")
cat("validate_cleaned_data()\n")
